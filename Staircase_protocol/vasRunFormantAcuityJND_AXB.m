...
prompt = {'Subject ID:',...
          'Session ID:',...
          'Tokens File:',...
          'Session Type: ("Cont" or "Pert"):',...
          'Perturber Type: ("NA", "BB", "LT"):',...
          'Gender ("male" or "female")'};
name = 'Subject Information';
numlines = 1;
defaultanswer = {'null', 'FA1', 'GT1', 'Cont', 'NA', 'female'};
...
num_trials = questdlg('Practice Set or Full Set?','Length','Practice','Full','Full') ;
switch num_trials
    case 'Practice'
        totalTrials = 20;
    case 'Full'
...
UD.sessType  = answer{4};
UD.pertType  = answer{5};
UD.gender    = answer{6};
...
% What are the Instructions Given to the participant? Depending on the
% task, we might ask them to identify same or different. We also track
% here, what were the options available to them to select.
UD.inst      = 'Diff';
...
UD.startValue    = 80; % initial difference in cents between speaker's fo and fo of stimulus in headphones
...
while (UD.stop == 0) && tr < UD.totalTrials
    ...
    set(ctrMsg, 'String', 'Different Sound', 'FontSize', 80)
...
expFiles = fullfile(dirs.RecFileDir, [UD.subject UD.run 'VAS.mat']);
switch num_trials
    case 'Practice'
        return
    case 'Full'
